...
$Excel.Visible = $true
$WorkBook = $Excel.Workbooks.Open("D:\coding\workplace\pshell\dlavala — дубли ФИО.xlsx")
...
$i = 2
while ($i -le $WorkSheet.UsedRange.Columns['B'].rows.Count) {
    #запись даты выдачи паспорта
    if ($WorkSheet.Columns['B'].rows[$i].Text -eq $WorkSheet.Columns['B'].rows[$i - 1].Text) {
        $WorkSheet.Columns['K'].rows[$i] = $WorkSheet.Columns['K'].rows[$i - 1].Text
        $WorkSheet.Columns['H'].rows[$i] = $WorkSheet.Columns['H'].rows[$i - 1].Text
        $WorkSheet.Columns['I'].rows[$i] = $WorkSheet.Columns['I'].rows[$i - 1].Text
        $i++
    } else {
    ...
    #запись номера паспорта
        if ($WorkSheet.Columns['I'].rows[$i].Text.Length -eq 0) {
            $randnum = Create-RandomNumber
            $WorkSheet.Columns['I'].rows[$i] = $randnum
        }
        $i++
    }
